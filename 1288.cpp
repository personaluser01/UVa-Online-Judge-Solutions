...
#define oo ((long double)(1<<29))
#define eps ((long double)(1e-4))
...
bool equal( long double a, long double b ) { return fabsl(a-b)<tol; };
bool leq( long double a, long double b ) { return equal(a,b) || a<b; };
bool geq( long double a, long double b ) { return equal(a,b) || a>b; };

const long double R = 6370.00, pi = 2*acos(0.00);
long double g[MAXV][MAXV],D;
long double S( long double s ) { return s*s; };
struct cell {
	long double x[DIM];
	cell() { for ( int i = 0; i < DIM; x[i++] = 0 ); };
	cell( long double phi, long double theta ) {
		x[0] = cos(theta)*cos(phi);
		x[1] = cos(theta)*sin(phi);
		x[2] = sin(theta);
	};
	cell( long double a, long double b, long double c ) { x[0] = a, x[1] = b, x[2] = c; };
...
void operator -= ( cell &a, const cell &b ) { for ( int i = 0; i < DIM; a.x[i] -= b.x[i], ++i ); };
void operator *= ( cell &a, const long double t ) { for ( int i = 0; i < DIM; a.x[i++] *= t );   };
void operator /= ( cell &a, const long double t ) { for ( int i = 0; i < DIM; a.x[i++] /= t );   };
...
long double operator ^ ( const cell &a, const cell &b ) {
	long double A = a.getlen(), B = b.getlen(),
		 		co = (a,b)/A/B;
	return acos(min((long double)1.00,max(co,(long double)-1.00)));
};
...
bool find_intersection( int idxm, int idxn, const cell &x, const cell &y, cell *res, bool is_great_circle ) {
	long double aa,bb,na,nb,t_v,mid,b[2],sw,_a,_b,_c,_d,discr;
	int biggest = 0;
	cell w,m[2];
...
bool covered( long double left, long double right, const vector<pair<long double, long double> > &e ) {
...
int main() {
	...
	long double hlen,full_tank,ll,rr,left,right,dw,fuel_left,nfuel,ans,ANG;
	cell tt[2];
#ifndef ONLINE_JUDGE
				...
				for ( map<cell,int>::iterator jt = s.begin(); jt != s.end(); ++jt )
					if ( (1 || jt->second >= n) && it->second != jt->second ) {
						if ( it->second >= n && jt->second >= n && belongsto[it->second] == belongsto[jt->second] ) {
							long double aang = it->first^jt->first;
...
#endif
						set<pair<long double,long double> > q;
						...
						DBG && puts("\n-------------------------------------\n");
						vector<pair<long double,long double> > e; e.clear();
						left = +oo, right = -oo;
						for ( set<pair<long double,long double> > :: iterator kt = q.begin(); kt != q.end(); ++kt ) {
							if ( kt->first > right ) {
								if ( left < +oo )
