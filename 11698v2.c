...
i64 p[N],delta[N*N],beta[N*N],C[N][N],F[N],Delta[N*N],Q,pr[N],D,
	z[N][N*N],alpha[N*N],kappa[N*N],*qtr,q[N*N];
double LO[N];
int PR;
divisor d[N*N],*ptr;
...
int main() {
	...
	for ( scanf("%lld",&ts); ts-- && 2 == scanf("%lld %lld",&n,&m); ) {
		...
		for ( z[0][0] = z[1][0] = 1, k = 2; k <= n; ++k )
			for ( i = 0; i < D; ++i ) 
				if ( d[i].p <= k ) {
					for ( mm = d[i].x, l = 0; l < Q; ++l )
						for ( kappa[l] = 0; !(mm % pr[l]); mm /= pr[l], ++kappa[l] );
					qtr = q, gener(0,1LL), nq = qtr-q;
					for ( tt = 0; tt < nq; ++tt ) q[tt] = find_which(q[tt]);
					for ( j = 0; j < nq; ++j )
						if ( d[q[j]].x <= k ) {
							for ( tt = 0; tt < nq; ++tt ) {
								if ( !z[k-d[q[j]].x][q[tt]] )
									continue ;
								if ( LCM(d[q[j]].x,d[q[tt]].x) != d[i].x )
									continue ;
								z[k][i] += (((((C[k-1][d[q[j]].x-1]*F[d[q[j]].x-1]) % MOD)*z[k-d[q[j]].x][q[tt]])) % MOD);
								z[k][i] %= MOD;
							}
							/*
							for ( l = 0; l <= i; ++l )
